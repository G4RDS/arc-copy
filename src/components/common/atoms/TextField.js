...
const TextField = props => {
  ...
  // typeがpasswordなら、パスワード中身表示時にtextにする
  let type = 'text'
  if (props.type === 'password' && isPasswordHidden) {
    type = 'password'
  }
  ...
  return (
    <Container className={props.className} fullWidth={props.fullWidth}>
      ...
      />
      {props.type === 'password' && (
        <ToggleVisibility isFocused={isFocused} hasError={!!errorMessage}>
          <IconButton
            title="パスワードの表示を切り替え"
...
const ToggleVisibility = styled.div`
  ...
  color: ${({ isFocused, hasError }) =>
    hasError ? colors.error[500] : isFocused ? colors.primary[500] : null};
`
